...
knitr::purl('tidyr.Rmd', documentation = F)
```
...
Learn more about tidy data: `vignette("tidy-data")`
...
Visit [tidyverse](https://www.tidyverse.org/) and `vignette("tidy-data")` for more examples on reshaping data set.  


## Pivot data

*As a package evolves, some function may turn **depreciated**. The function may still work but it would be recommended to shift for the new function performing the task. The package `tidyr` is a good example of this evolution. The function `gather` is depreciated (I am getting older and I keep using old function), and it is now recommended to switch to `pivot_longer()` still under active development. Thus `df %>% gather("key", "value", x, y, z)`is now equivalent to `df %>% pivot_longer(c(x, y, z), names_to = "key", values_to = "value")`.
...
Here’s a subset of median income by gender and by work experience for 2017 in four states in US and Taiwan (real data).
...
From the `tidyr`, check `uncount()` replicate rows by count, `unite()` combining elements from many columns into a single column, etc.  You have many  documentation existing online online on the use of this package. It is a very convenient pcakage when you have to repeat a formatting task on a large data set.
