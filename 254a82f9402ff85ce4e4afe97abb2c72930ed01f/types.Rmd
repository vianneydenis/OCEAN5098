...
knitr::purl('types.Rmd', documentation = F)
```
...
**為什麼**, how can it be  ? Well, `a.fact` is a more complicated object than the simple objects created thus far in that the factor is storing additional information not seen in its output. This hidden information is stored in **attributes**. To view these hidden attributes, use the `attributes` function.
...
The **atomic vector** (or **vector** for short) is the simplest data structure in R which consists of an ordered set of values of the same type and or class (e.g. numeric, character, date, etc…). As we did it before, a vector can be created using the combine function `c()` as in:
...
<p class="alert">
**`r knitr::asis_output("\U26A0")` Practice 2:** Create the following data frame from scratch. This short experiment aims at testing the efficiency of a diet made of bubble teas: three cups a day for a week. Weight of subjects is provided before and after this 'bubble tea' diet.  Reformat this data frame to obtain the `weight` as a double numeric into one column and the `time` of measurement as factor with two levels `before_diet` and `after_diet`. You will store this **data frame** as the first element of a list called **`BUBBLE_DIET`**. The second element of this list will be another list called **`WEIGHT_LOSS`** with three elements: [1] a **vector of character** extracting the row names of the table previously created; [2] a **numeric vector** (double) indicating the weight loss (in %) of each subject (can be positive or negative); [3] combine these two elements in a **table with two columns**: `subject` and `weight_loss`. This will be you third element of the `WEIGHT_LOSS` list. Push both your *.Rmd* and *.html* files into a public repository available from your Github account.You will share with me be email [vianneydenis@g.ntu.edu.tw] the address (URL) of this repository (such as https://github.com/vianneydenis/OCEAN5098.git) **before next Monday** in order for me to check your work. The **title of your email ** should be `Practice 2 (your name: your student no.). ENJOY ;)
</p>
