...
Summarizing your data set and examining its structure allow you to an error in your data set (including format errors that may have been caused at the importation of data set, remember `reef_fish.txt`). You can fix format error using `read.table` or others. Alternatively, you can `fix` it to open a spreadsheet:
...
Our subset is constituted of a selection of the five female students. The original `students` data set had ten observations, and I can check this change calling for the dimensions of the new object `females` and using `dim(females)`. The new data set is a `data.frame` of 5 observations x 4 columns. Column names `colnames(females)` remained unchanged, but I have now only 5 names for the rows `rownames(females)`. I can the rows by  combining `c` the names of the female students into a **vector**.
